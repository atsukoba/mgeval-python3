...
from mgeval.core import MgevalMetrics, extract_feature
from mgeval.utils import kl_dist, overlap_area, c_dist
...
single_arg_metrics = (
    ["total_used_pitch", "avg_IOI", "total_pitch_class_histogram", "pitch_range"
     ])
...
for _set, _set_eval in sets:
    for i in range(0, num_samples):
        ...
        for metric in metrics_list:
            evaluator = getattr(MgevalMetrics(), metric)
...
for i, metric in enumerate(metrics_list):
    for train_index, test_index in loo.split(np.arange(num_samples)):
        sets_inter[test_index[0]][i] = c_dist(
