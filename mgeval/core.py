...
from python3_midi import Pattern


class Feature:
    def __init__(self, pretty_midi: PrettyMIDI, midi_pattern: Pattern, **kwargs):
        self.pretty_midi = pretty_midi
        self.midi_pattern = midi_pattern
        for k, v in kwargs.items():
            self.__setattr__(k, v)


def extract_feature(_file, program_sort: bool = True, drums: bool = False):
    """
    ...
    pm = PrettyMIDI(_file)
    pm.instruments = list(
        filter(lambda i: not (i.is_drum ^ drums), pm.instruments))
    if program_sort:
        pm.instruments.sort(key=lambda i: i.program)
    feature: Feature = Feature(pm, midi.read_midifile(_file))
    return feature


# musically informed objective measures.
class MgevalMetrics(object):

    def __init__(self):
        pass

    def total_used_pitch(self, feature: Feature, track_num: int = 0):
        """
        ...
        """
        inst: Instrument = feature.pretty_midi.instruments[track_num]
        piano_roll = inst.get_piano_roll(fs=100)
        sum_notes = np.sum(piano_roll, axis=1)
        used_pitch = np.sum(sum_notes > 0)
        return used_pitch

    def bar_used_pitch(self, feature: Feature, track_num: int = 0, num_bar: Optional[int] = None):
        """
        ...
        for i in range(0, len(pattern[track_num])):
            if type(pattern[track_num][i]) == midi.events.TimeSignatureEvent:
                ...
                if num_bar is None:
                    num_bar = int(
                        round(float(pattern[track_num][-1].tick) / bar_length))
        ...
        for i in range(0, num_bar):
            used_pitch[i] = len(
                set(note_list[current_note:current_note + int(used_notes[i][0])]))
            current_note += int(used_notes[i][0])

        return used_pitch

    def total_used_note(self, feature: Feature, track_num: int = 0):
        """
    ...
    def bar_used_note(self, feature: Feature, track_num: int = 0, num_bar=None):
        """
    ...
    def total_pitch_class_histogram(self, feature: Feature, track_num: int = 0):
        """
    ...
    def bar_pitch_class_histogram(self, feature: Feature, track_num: int = 0, num_bar=None, bpm=120):
        """
        ...
        if num_bar is None:
            ...
            piano_roll = pm_object.instruments[track_num].get_piano_roll(
                fs=100)
        ...
        if actual_bar > num_bar:
            ...
            # make exact bar
            piano_roll = piano_roll[:-np.mod(len(piano_roll),
                                             bar_length)].reshape((num_bar, -1, 128))
        elif actual_bar == num_bar:
            piano_roll = piano_roll.reshape((num_bar, -1, 128))
        else:
            piano_roll = np.pad(piano_roll, ((0, int(
                num_bar * bar_length - len(piano_roll))), (0, 0)), mode="constant", constant_values=0)
            piano_roll = piano_roll.reshape((num_bar, -1, 128))
    ...
    def pitch_class_transition_matrix(self, feature: Feature, normalize=0, track_num: int = 0):
        """
        ...
        """
        inst: Instrument = feature.pretty_midi.instruments[track_num]
        transition_matrix: np.ndarray = inst.get_pitch_class_transition_matrix()

        if normalize == 0:
            return transition_matrix

        elif normalize == 1:
            sums: np.ndarray = np.sum(transition_matrix, axis=1)
            sums[sums == 0] = 1
    ...
    def pitch_range(self, feature: Feature, track_num: int = 0):
        """
    ...
    def avg_pitch_shift(self, feature: Feature, track_num: int = 0):
        """
    ...
    def avg_IOI(self, feature: Feature, track_num: int = 0):
        """
        ...
        inst: Instrument = feature.pretty_midi.instruments[track_num]
        onset = inst.get_onsets()
        ioi = np.diff(onset)
        avg_ioi = np.mean(ioi)
        return avg_ioi

    def note_length_hist(self, feature: Feature, track_num: int = 0, normalize=True, pause_event=False):
        """
    ...
    def note_length_transition_matrix(self, feature: Feature, track_num: int = 0, normalize=0, pause_event=False):
        """
    ...
    # def chord_dependency(self, feature: Feature, bar_chord, bpm=120, num_bar=None, track_num: int=0):
    #     pm_object = feature.pretty_midi
